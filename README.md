...
#### Demo of tutorials generated by SourceLens 
...
2.  **Install `sourceLens`:**
    ...
    ```bash
    pip install https://github.com/openXFlow/sourceLensAI/releases/download/v0.3.0/sourcelens-0.3.0-py3-none-any.whl
    ```
...
*   **[Index of Local Code Project Analyses](./output/index_Code_Analysis_Local_Projects.md)**
    ...
    *   **Programming Languages**: Analysis of simple applications written in `Python`, `C`, `C++`, `C#`, `Java`, `JavaScript`, `TypeScript`, `PHP`, `Ruby`, `Swift`, `Go`, and `Rust`, demonstrating multi-language outputs (English, German, Spanish, Japanese, etc.).
    *   **Hardware & Scientific**: Examples include `Analog & Digital` circuit designs (SPICE, Verilog) and a `MATLAB` signal processing project.
    *   **Data & Scripting**: Analysis of a `SQL Database Schema` and a collection of `Shell Scripts` (Bash, PowerShell).
    *   **DevOps & Web Frontend**: Demonstrates understanding of Infrastructure-as-Code with `Terraform/Ansible`, `Kubernetes YAML` configurations, and a `Vue.js` frontend project.

*   **[Index of GitHub Repository Analyses](./output/index_Code_Analysis_GitHub_Repository.md)**
    *   Lists all tutorials generated from remote GitHub repositories, including real-world projects like the **Flask** web framework and the **SourceLens** project itself.

*   **[Index of Web Content & YouTube Analyses](./output/index_Web_Content_Analysis.md)**
    *   Lists all summaries and analyses generated from web sources, showcasing the ability to generate summaries from both technical documentation sites (`Crawl4AI`) and **YouTube video transcripts**.
