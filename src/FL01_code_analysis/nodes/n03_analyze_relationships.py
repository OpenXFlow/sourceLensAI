...
"""
AnalyzeRelationshipsExecutionResult: TypeAlias = CodeRelationshipsDict
"""Type alias for the execution result. Keys: 'summary', 'details' (list of CodeRelationshipDetailItem)."""

# TypeAlias pre RawLLMIndexEntry zostáva, je interný pre logiku parsovania
...
class AnalyzeRelationships(BaseNode[AnalyzeRelationshipsPreparedInputs, AnalyzeRelationshipsExecutionResult]):
    ...
    def _parse_single_relationship(
    ...
    ) -> Optional[tuple[CodeRelationshipDetailItem, set[int]]]:
        ...
        Returns:
            A tuple (CodeRelationshipDetailItem, set of involved indices) if valid, otherwise None.
        """
        try:
            ...
            relationship_detail: CodeRelationshipDetailItem = {"from": from_idx, "to": to_idx, "label": label}
    ...
    def _parse_and_validate_relationships(
    ...
    ) -> tuple[list[CodeRelationshipDetailItem], set[int]]:
        ...
        Returns:
            A tuple containing a list of validated `CodeRelationshipDetailItem` and a set of involved indices.
        """
        validated_relationships: list[CodeRelationshipDetailItem] = []
    ...
    def _build_relationship_context(
        self, abstractions: CodeAbstractionsList, files_data: FilePathContentList
    ) -> tuple[str, str]:
    ...
    def pre_execution(self, shared_context: SLSharedContext) -> AnalyzeRelationshipsPreparedInputs:
        ...
        abstractions: CodeAbstractionsList = abstractions_any if isinstance(abstractions_any, list) else []
    ...
    def _process_validated_yaml_data(
        ...
        Returns:
            An `AnalyzeRelationshipsExecutionResult` dictionary (which is CodeRelationshipsDict).
        """
        default_output: AnalyzeRelationshipsExecutionResult = {"overall_summary": DEFAULT_ERROR_SUMMARY, "details": []}
        if not isinstance(relationships_data_yaml, dict):
            ...
            default_output["overall_summary"] = "Failed to obtain or validate relationship data from LLM."
        ...
        return {"overall_summary": summary_text, "details": valid_rels}
    ...
    def execution(self, prepared_inputs: AnalyzeRelationshipsPreparedInputs) -> AnalyzeRelationshipsExecutionResult:
        ...
        default_output: AnalyzeRelationshipsExecutionResult = {
            "overall_summary": "Analysis not performed.",
            "details": [],
        }

        if num_abstractions == 0:
            self._log_info("Skipping relationship analysis due to no abstractions.")
            default_output["overall_summary"] = "No abstractions were available to analyze."
            return default_output
        ...
        if validated_yaml_data is None:
            default_output["overall_summary"] = "Failed to get valid relationship data from LLM."
            return default_output
        ...
        try:
            ...
            default_output["overall_summary"] = "Critical error processing relationship data after validation."
            return default_output
    ...
    def post_execution(
        ...
        summary_snippet_raw: Any = execution_outputs.get("overall_summary", "")
...
# End of src/FL01_code_analysis/nodes/n03_analyze_relationships.py
