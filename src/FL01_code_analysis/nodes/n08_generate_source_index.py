...
from sourcelens.mermaid_diagrams.file_structure_diagram import generate_file_structure_mermaid
from sourcelens.utils import LlmApiError

from .index_formatters import (
...
# FileDataInternal and FileDataListInternal are replaced by OptionalFilePathContentList

EXPECTED_FILE_DATA_TUPLE_LENGTH_SRC_IDX: Final[int] = 2  # This constant seems specific enough to stay
...
class GenerateSourceIndexNode(BaseNode[SourceIndexPreparedInputs, SourceIndexExecutionResult]):
    ...
    def _generate_detailed_content_ast(
        self,
        project_name: str,
        files_data_for_ast: OptionalFilePathContentList,
        ...
        python_files_to_process: OptionalFilePathContentList = [
    ...
    def pre_execution(self, shared_context: SLSharedContext) -> SourceIndexPreparedInputs:
        ...
        try:
            ...
            source_config: "ResolvedSourceConfigDict" = source_config_val if isinstance(source_config_val, dict) else {}
            llm_config: "ResolvedLlmConfigDict" = llm_config_val if isinstance(llm_config_val, dict) else {}
            cache_config: "ResolvedCacheConfigDict" = cache_config_val if isinstance(cache_config_val, dict) else {}
            ...
            valid_files_data: OptionalFilePathContentList = []
    ...
    def execution(self, prepared_inputs: SourceIndexPreparedInputs) -> SourceIndexExecutionResult:
        ...
        files: OptionalFilePathContentList = prepared_inputs["files_data"]
        ...
        llm_cfg: "ResolvedLlmConfigDict" = prepared_inputs["llm_config"]
        cache_cfg: "ResolvedCacheConfigDict" = prepared_inputs["cache_config"]
        ...
        if not files and (parser == "ast" or parser == "llm"):
...
# End of src/FL01_code_analysis/nodes/n08_generate_source_index.py
