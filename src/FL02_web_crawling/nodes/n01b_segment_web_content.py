...
from sourcelens.core.common_types import WebContentChunk
...
MAX_HEADING_LEVEL: Final[int] = 6  # Max valid Markdown heading level
...
class SegmentWebContent(BaseNode[SegmentWebContentPreparedInputs, SegmentWebContentExecutionResult]):
    ...
    def _create_dynamic_regexes(self, heading_levels: list[int]) -> tuple[re.Pattern[str], re.Pattern[str]]:
        ...
        if not heading_levels:
            levels = DEFAULT_HEADING_LEVELS_TO_SPLIT_ON
        else:
            # Filter for valid heading levels (1-6) and remove duplicates
            levels = sorted(set(h_level for h_level in heading_levels if 1 <= h_level <= MAX_HEADING_LEVEL))
            if not levels:  # Fallback if provided levels were all invalid
                levels = DEFAULT_HEADING_LEVELS_TO_SPLIT_ON

        levels_str = "".join(map(str, levels))  # sorted set ensures order for regex

        heading_regex_str = rf"(^#{{{levels_str}}})\s+([^\n]+)\n(.*?)(?=(^#{{{levels_str}}}\s+[^\n]+$)|\Z)"
        initial_content_regex_str = rf"^(.*?)(?=(^#{{{levels_str}}}\s+[^\n]+$)|\Z)"
    ...
    def _yield_initial_or_full_content_chunk(
        ...
        """Yield chunk for initial content or full content if no headings found."""
        current_file_stem = sanitize_filename(Path(filepath).stem)
        match = initial_content_regex.match(content)
        ...
        if match:
            ...
            if initial_text and len(initial_text) >= min_chunk_len:
                chunk_id = f"{current_file_stem}_initial_content_0"
                self._log_debug(
            ...
            if match.group(2) is None and match.end() == len(content):
                if chunk_to_yield is None and len(content.strip()) >= min_chunk_len:
                    chunk_id = f"{current_file_stem}_full_content_0"
                    self._log_debug(
                        "Yielding full_content chunk (no headings): ID='%s', Title='%s', Length=%d",
                        chunk_id,
                        Path(filepath).name,
                        len(content.strip()),
                    )
    ...
    def _yield_section_chunks(
        ...
        """Yield chunks for sections found by heading_regex."""
        current_file_stem = sanitize_filename(Path(filepath).stem)
        chunks: list[WebContentChunk] = []
        last_pos_processed = start_offset
        chunk_index_offset = 1 if start_offset > 0 else 0

        for match in heading_regex.finditer(content, pos=start_offset):
            heading_level_str = match.group(1)
            heading_text = match.group(2).strip()
            section_content_text = match.group(3).strip()

            if section_content_text and len(section_content_text) >= min_chunk_len:
                chunk_id_base = sanitize_filename(heading_text) or f"section_{chunk_index_offset + len(chunks)}"
                chunk_id = f"{current_file_stem}_{chunk_id_base}"
                self._log_debug(
                ...
                self._log_debug(
                    "Skipping section '%s' in '%s': len %d < min %d",
                    heading_text,
                    filepath,
                    len(section_content_text),
                    min_chunk_len,
    ...
    def _yield_final_content_chunk(
        ...
        """Yield chunk for content after the last heading, if significant."""
        current_file_stem = sanitize_filename(Path(filepath).stem)
        remaining_content = content[last_pos_processed:].strip()
        if remaining_content and len(remaining_content) >= min_chunk_len:
            chunk_id = f"{current_file_stem}_final_content_{num_existing_chunks}"
            self._log_debug(
    ...
    def _generate_chunks_from_content(
        ...
        """Generate chunks from a single document's content. (Main orchestrator)."""
        ...
        final_chunk = self._yield_final_content_chunk(filepath, content, min_chunk_len, current_pos, num_chunks_yielded)
        if final_chunk:
            yield final_chunk
    ...
    def pre_execution(self, shared_context: SLSharedContext) -> SegmentWebContentPreparedInputs:
        ...
        flow_name = str(shared_context.get("current_operation_mode", "FL02_web_crawling"))
        flow_config = config_data.get(flow_name, {})
        segmentation_opts = flow_config.get("segmentation_options", {})

        seg_enabled = bool(segmentation_opts.get("enabled", True))
        if not seg_enabled:
            self._log_info("Segmentation is disabled in configuration. Skipping segmentation node.")
            shared_context["web_content_chunks"] = []
            return []

        min_chunk_len = int(segmentation_opts.get("min_chunk_char_length", DEFAULT_MIN_CHUNK_CHAR_LENGTH))
        heading_levels_raw = segmentation_opts.get("heading_levels_to_split_on", DEFAULT_HEADING_LEVELS_TO_SPLIT_ON)
        # Filter for valid heading levels (1-MAX_HEADING_LEVEL) and remove duplicates, then sort
        ...
        self._log_info(
            "Segmentation parameters: enabled=%s, min_chunk_length=%d, heading_levels=%s",
            seg_enabled,
            min_chunk_len,
            heading_levels,
        )

        segmentation_params = {"min_chunk_len": min_chunk_len, "heading_levels": heading_levels}
        ...
        for item in files_data_any:
            if (
                ...
                filepath, file_content = item
                if file_content.strip():
                    prepared_data.append((filepath, file_content, segmentation_params))
                else:
                    self._log_debug("Skipping file '%s' for segmentation as it has no content.", filepath)
            else:
                self._log_warning(
    ...
    def execution(self, prepared_inputs: SegmentWebContentPreparedInputs) -> SegmentWebContentExecutionResult:
        """Segment the content of each fetched web document."""
        ...
        for filepath, content, seg_params in prepared_inputs:
            min_len = seg_params["min_chunk_len"]
            levels = seg_params["heading_levels"]
            self._log_debug("Segmenting content from: %s with min_len=%d, levels=%s", filepath, min_len, levels)
            try:
                file_chunks = list(self._generate_chunks_from_content(filepath, content, min_len, levels))
    ...
    def post_execution(
        ...
        """Store the list of web content chunks in the shared context."""
        del prepared_inputs
