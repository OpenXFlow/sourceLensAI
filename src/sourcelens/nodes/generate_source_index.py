...
class GenerateSourceIndexNode(BaseNode[SourceIndexPrepResult, SourceIndexExecResult]):
    ...
    def _append_class_md(self, lines: list[str], class_name: str, class_data: ClassStructure) -> None:
        ...
        attributes: list[ClassAttributeStructure] = class_data.get("attributes", [])  # type: ignore[assignment]
        methods: list[ClassMethodStructure] = class_data.get("methods", [])  # type: ignore[assignment]
    ...
    def _format_file_entry(self, file_display_path: str, structure: ModuleStructure, file_number: int) -> list[str]:
        ...
        module_functions: list[FunctionStructure] = structure.get("functions", [])  # type: ignore[assignment]
        classes: dict[str, ClassStructure] = structure.get("classes", {})  # type: ignore[assignment]
    ...
    def _format_detailed_file_entries(
        ...
        for _, (path_from_files_data, content_opt) in enumerate(files_data):  # 'i' was unused
            processed_file_count += 1
            # Original code had a validation for item structure which is implicitly handled
            # by iterating over files_data which is FileDataList (list[tuple[str, Optional[str]]])
            ...
            if path_from_files_data.endswith((".py", ".pyi")):
                ...
                else:  # Python file with no content (e.g., read error)
    ...
    def _format_generic_index(self, project_name: str, files_data: FileDataList, project_scan_root_display: str) -> str:
        ...
        markdown_lines.append("\n## File List\n")
        if files_data:
            for _, (path_rel_to_scan_root, content_str_opt) in enumerate(files_data):  # 'i' was unused
    ...
    def exec(self, prep_res: SourceIndexPrepResult) -> SourceIndexExecResult:
        ...
        # Ensure keys exist before accessing, with type checking/casting
