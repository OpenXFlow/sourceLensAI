...
# Import the new function for generating file structure Mermaid
...
"""Type alias for the execution result (Markdown content or None)."""
...
class PythonCodeVisitor(ast.NodeVisitor):
    ...
    def _get_annotation_str(self, node: Optional[ast.expr], *, is_self_arg: bool = False) -> str:
        ...
        if node is None:
            return ""
        # Attempt to get source segment first
        if self._source_code_lines and hasattr(node, "lineno"):
            try:
                # Ensure source_code_lines is a single string for get_source_segment
            ...
            except (AttributeError, TypeError, ValueError, IndexError) as e_segment:  # Catch specific errors
                visitor_logger.debug(
                    "Failed to get source segment for annotation node (line %s): %s. Falling back to AST parsing.",
                    getattr(node, "lineno", "N/A"),
                    str(e_segment),
                )
        # Fallback to AST node parsing if source segment fails or not available
    ...
    def _format_arguments(self, args_node: ast.arguments) -> str:
        ...
        if args_node.posonlyargs:
            args_list.append("/")  # PEP 570 separator

        # Regular arguments (positional or keyword)
        for i, arg in enumerate(args_node.args):
            is_self_or_cls = (
                arg.arg in {"self", "cls"}
                and (i == 0 and not args_node.posonlyargs)  # It's the first arg and no posonly args before it
                and self._current_class_name is not None  # And we are inside a class
            )
        ...
        if args_node.kwonlyargs:
            if not args_node.vararg:  # If no *args, need a bare * to indicate kwonly start
                args_list.append("*")
        ...
        return ", ".join(filter(None, args_list))  # Filter out empty strings if any part was optional and None
    ...
    def _get_decorator_name_str(self, decorator_func_node: ast.expr) -> str:
        ...
        # Fallback if source segment parsing is needed and available
        try:
        ...
        return "..."  # Fallback for complex decorators not easily represented
    ...
    def _get_decorator_str(self, decorator_node: ast.expr) -> str:
        ...
        """
        # Try to get source segment first for better accuracy
        try:
        ...
        if isinstance(decorator_node, ast.Call):
            ...
            # For simplicity, don't try to reconstruct arguments, just indicate it's a call
            return f"@{func_str}(...)"
        return "@..."  # Fallback for other complex decorator structures
    ...
    def visit_ClassDef(self, node: ast.ClassDef) -> None:
        """Visit a ClassDef node and extract its details.
        ...
        self._current_class_name = class_name  # Set context for method visits

        class_data = self.classes[class_name]  # Uses defaultdict
        ...
        for child_node in node.body:
            if isinstance(child_node, ast.AnnAssign) and isinstance(child_node.target, ast.Name):
                # Annotated assignment (class or instance variable with type hint)
                ...
                child_node, ast.Assign
            ):  # Simple assignment (instance variable without class-level type hint)
                for target_node in child_node.targets:  # Can be multiple targets: a = b = 1
                ...
                # This will call visit_FunctionDef or visit_AsyncFunctionDef
                self.visit(child_node)  # Recursive call for methods

        self._current_class_name = original_current_class_name_context  # Restore context

    def visit_FunctionDef(self, node: ast.FunctionDef) -> None:
        """Visit a FunctionDef node and extract its details.
        ...
        if self._current_class_name:  # If we are inside a class, it's a method
            self.classes[self._current_class_name]["methods"].append({"signature": signature, "doc": doc})
        else:  # Otherwise, it's a module-level function
            self.functions.append({"signature": signature, "doc": doc})

    def visit_AsyncFunctionDef(self, node: ast.AsyncFunctionDef) -> None:
        """Visit an AsyncFunctionDef node and extract its details.
        ...
        if self._current_class_name:  # If we are inside a class, it's an async method
            self.classes[self._current_class_name]["methods"].append({"signature": signature, "doc": doc})
        else:  # Otherwise, it's a module-level async function
            self.functions.append({"signature": signature, "doc": doc})
    ...
    def parse(self, code: str) -> ModuleStructure:
        ...
        Returns:
            A `ModuleStructure` dictionary containing parsed information
            and any parsing error.
        """
        self._source_code_lines = code.splitlines(keepends=True)  # Keep newlines for ast.get_source_segment
        self.module_doc = None
        self.classes.clear()
        ...
        except Exception as e_general_parse:  # Catch other potential parsing issues
            parsing_error_message = f"AST Parsing Error: {str(e_general_parse)}"
            visitor_logger.error("An unexpected error occurred during AST parsing: %s", e_general_parse, exc_info=True)

        return {
            "doc": self.module_doc or "",
            "classes": dict(self.classes),  # Convert defaultdict to dict for output
...
class GenerateSourceIndexNode(BaseNode[SourceIndexPrepResult, SourceIndexExecResult]):
    ...
    def _append_module_functions_md(
        ...
        if module_functions:
            ...
            if lines and lines[-1].strip():  # Add newline if previous line wasn't empty
                lines.append("\n")
            for func_info in sorted(module_functions, key=lambda x: x.get("signature", "")):
                ...
                lines.append(f"*   **`{signature}:`**\n")  # Ensure newline after signature
                if doc:
                    lines.append(f"    ... {doc}\n")  # Ensure newline after doc
    ...
    def _append_class_md(self, lines: list[str], class_name: str, class_data: ClassStructure) -> None:
        ...
        if lines and lines[-1].strip() and not lines[-1].endswith(("\n\n", "\n")):
            lines.append("\n\n")  # Ensure two newlines before a new class section if needed
        ...
        lines.append("\n")  # Add a blank line after class docstring

        attributes_any: Any = class_data.get("attributes", [])
        methods_any: Any = class_data.get("methods", [])
        attributes: list[ClassAttributeStructure] = attributes_any if isinstance(attributes_any, list) else []
        methods: list[ClassMethodStructure] = methods_any if isinstance(methods_any, list) else []

        if attributes:
            self._append_class_attributes_md(lines, attributes)
        if methods:
            if attributes:  # Add a blank line between attributes and methods if both exist
                lines.append("\n")
    ...
    def _format_file_entry(self, file_display_path: str, structure: ModuleStructure, file_number: int) -> list[str]:
        ...
        lines: list[str] = ["\n##\n"]  # Start with a horizontal rule (or clear separator)
        p = Path(file_display_path)
        parent_dir_for_h6 = p.parent.as_posix()
        if parent_dir_for_h6 == ".":
            parent_dir_for_h6 = "./"  # Consistent display for root
        elif parent_dir_for_h6 and not parent_dir_for_h6.endswith("/"):
            parent_dir_for_h6 += "/"
        elif not parent_dir_for_h6:  # Should not happen with Path.parent but good practice
            parent_dir_for_h6 = "./"

        lines.append(f"###### {file_number}) {parent_dir_for_h6}\n")
        lines.append(f"#  {p.name}\n")

        parsing_error: Optional[str] = structure.get("parsing_error")  # type: ignore[assignment]
        module_doc: str = structure.get("doc", "")  # type: ignore[assignment]
        module_functions_any: Any = structure.get("functions", [])
        classes_any: Any = structure.get("classes", {})

        module_functions: list[FunctionStructure] = (
            module_functions_any if isinstance(module_functions_any, list) else []
        )
        classes: dict[str, ClassStructure] = classes_any if isinstance(classes_any, dict) else {}

        if parsing_error:
            lines.append(f"... Error parsing this file: {parsing_error}\n")
        elif module_doc:
            lines.append(f"... {module_doc}\n")
        elif not classes and not module_functions:  # Neither functions nor classes, and no module doc
            lines.append("... (No parseable top-level functions or classes, and no module docstring found)\n")
        elif not module_doc:  # Has functions/classes but no module doc
        ...
        if lines and lines[-1].strip():  # Ensure last line is a newline if content was added
            lines.append("\n")
    ...
    def _format_file_summary_list(self, files_data: FileDataList, visitor: PythonCodeVisitor) -> list[str]:
        ...
        for path_rel_to_scan_root, file_content_str_opt in files_data:
            ...
            if path_rel_to_scan_root.endswith((".py", ".pyi")) and file_content_str_opt:
            ...
            elif file_content_str_opt:  # For non-python files, try to grab first line as summary
                first_line = file_content_str_opt.strip().split("\n", 1)[0].strip()
                # Simple heuristic for comment or docstring-like first line
                ...
                else:  # Otherwise, just use the first line (truncated)
    ...
    def _format_detailed_file_entries(
        ...
        for i, (path_from_files_data, content_opt) in enumerate(files_data):
            ...
            # Determine display path (relative to scan root, not CWD)
            ...
            if path_from_files_data.endswith((".py", ".pyi")):
                ...
                else:  # Python file but no content (should ideally not happen if fetched correctly)
    ...
    def _format_python_index(self, project_name: str, files_data: FileDataList, project_scan_root_display: str) -> str:
        ...
        if files_data:
            # Use the imported generate_file_structure_mermaid
        ...
        visitor = PythonCodeVisitor()  # Initialize visitor once for all Python files
        ...
        elif not files_data:  # If files_data itself was empty
            markdown_lines.append("\n\nNo files found in the source to index.\n")

        result: str = "".join(markdown_lines)
        self._log_info(
            "Finished Python source index for '%s'. Files processed: %d. Total length: %d",
            project_name,
            processed_count,
            len(result),
        )
    ...
    def _format_non_python_file_entry(
        ...
        if content and content.strip():
            ...
            # Heuristic for comments or docstring-like lines
            ...
            elif first_line_doc_raw.startswith("#"):  # Common comment prefix
    ...
    def _format_generic_index(self, project_name: str, files_data: FileDataList, project_scan_root_display: str) -> str:
        ...
        if files_data:
            for i, (path_rel_to_scan_root, content_str_opt) in enumerate(files_data):
                # Use project_scan_root_display to construct the display path
                ...
                markdown_lines.append(
                    f"{i + 1}. **`{Path(file_display_path).name}`** (`{file_display_path}`){summary_for_list}\n"
                )
        else:
    ...
    def prep(self, shared: SLSharedState) -> SourceIndexPrepResult:
        ...
        try:
            ...
            llm_config_any: Any = self._get_required_shared(shared, "llm_config")  # Kept for future LLM parser
            cache_config_any: Any = self._get_required_shared(shared, "cache_config")  # Kept for future LLM parser
            ...
            project_scan_root_display: str = str(project_scan_root_display_val)
            # Normalize to ensure it acts like a directory prefix
            if project_scan_root_display and not project_scan_root_display.endswith("/"):
                project_scan_root_display += "/"
            elif not project_scan_root_display:  # Handles empty string or None
                project_scan_root_display = "./"
            ...
            self._log_info(
                "Effective config for prep: include_source_index=%s, scan_root_display='%s'",
            ...
            self._log_info("Decision for source_index details: language=%s, parser_type=%s", language, parser_type)

            # Ensure files_data contains tuples of (str, Optional[str])
            validated_files_data: FileDataList = []
            for item in files_data:
                if (
                    ...
                    validated_files_data.append(item)
                else:
                    self._log_warning("Skipping invalid item in files_data during prep: %s", item)
            ...
            self._log_error("Prep for source index failed (missing data): %s", e_val, exc_info=True)
            return {"skip": True, "reason": f"Missing shared data: {e_val!s}"}
        except Exception as e_prep:  # noqa: BLE001 - Broad exception for unexpected issues in prep
            self._log_error("Unexpected error during GenerateSourceIndexNode.prep: %s", e_prep, exc_info=True)
            return {"skip": True, "reason": f"Unexpected prep error: {e_prep!s}"}
    ...
    def exec(self, prep_res: SourceIndexPrepResult) -> SourceIndexExecResult:
        ...
        files_data_any: Any = prep_res.get("files_data")
        project_name_any: Any = prep_res.get("project_name")
        language_any: Any = prep_res.get("language")
        parser_type_any: Any = prep_res.get("parser_type")
        scan_root_any: Any = prep_res.get("project_scan_root_display")

        files_data: FileDataList = files_data_any if isinstance(files_data_any, list) else []
        project_name: str = str(project_name_any) if isinstance(project_name_any, str) else "Unknown Project"
        language: str = str(language_any) if isinstance(language_any, str) else "unknown"
        parser_type: str = str(parser_type_any) if isinstance(parser_type_any, str) else "none"
        scan_root: str = str(scan_root_any) if isinstance(scan_root_any, str) else "./"

        self._log_info(
            "Executing index generation for project '%s' (Lang:%s, Parser:%s, Files:%d, Scan Root Display:'%s')",
            project_name,
            language,
            parser_type,
            len(files_data),
        ...
        if not files_data:
            self._log_warning("No files data provided to generate source index, though not explicitly skipped.")
            return f"# Code Inventory: {project_name}\n\nNo files found in the source to index.\n"
        try:
            markdown_content: str
            if parser_type == "ast" and language == "python":
                self._log_info("Calling _format_python_index (AST based).")
                markdown_content = self._format_python_index(project_name, files_data, scan_root)
            elif parser_type == "llm":  # This branch is currently a fallback
                self._log_warning(
                    "LLM-based source index for language '%s' is not yet implemented. Using generic list.", language
                )
                markdown_content = self._format_generic_index(project_name, files_data, scan_root)
            else:  # "none" or any other parser type
                self._log_info("Using generic file list for source index (parser_type: '%s').", parser_type)
                markdown_content = self._format_generic_index(project_name, files_data, scan_root)
            return markdown_content if markdown_content.strip() else None
        except Exception as e_exec_format:  # noqa: BLE001 - Catch unexpected errors during formatting
            self._log_error("Unexpected error formatting source index: %s", e_exec_format, exc_info=True)
    ...
    def post(
        ...
        # Log prep_res details for better debugging
        if isinstance(prep_res, dict):
            prep_summary = {
                k: (len(v) if isinstance(v, list) else v)
                for k, v in prep_res.items()
                if k != "files_data"  # Avoid logging potentially large file data
            }
            ...
            self._log_info("Post called. prep_res type: %s", type(prep_res).__name__)

        shared["source_index_content"] = None  # Default to None

        self._log_info("exec_res type: %s", type(exec_res).__name__)
        if isinstance(exec_res, str):
            self._log_info("exec_res length: %d", len(exec_res))
            if exec_res.strip():
                shared["source_index_content"] = exec_res
                self._log_info("Stored VALID source index content (length: %d).", len(exec_res))
            else:
                self._log_warning("Source index content from exec was empty string. Not storing.")
        else:  # exec_res is None
            self._log_warning("Source index content from exec was None. Not storing.")
