...
WEB_INVENTORY_CHAPTER_TITLE: Final[str] = "Content Inventory"
...
class CombineWebSummary(BaseNode[CombineWebSummaryPreparedInputs, CombineWebSummaryExecutionResult]):
    ...
    def _get_special_chapter_base_name(self, chapter_type: WebChapterTypeLiteral) -> str:
        ...
        Args:
            chapter_type (WebChapterTypeLiteral): The type of the special chapter.

        Returns:
            str: The base filename string for that chapter type.
        """
    ...
    def _add_navigation_links(self, all_chapters_data: AllWebChaptersDataList, index_filename: str) -> None:
        ...
        Args:
            all_chapters_data (AllWebChaptersDataList): List of all chapter data dictionaries.
            index_filename (str): The filename of the main index/overview page.
        """
    ...
    def _add_footers(self, all_chapters_data: AllWebChaptersDataList, footer_text: str) -> None:
        ...
        Args:
            all_chapters_data (AllWebChaptersDataList): List of all chapter data dictionaries.
            footer_text (str): The footer text to append.
        """
    ...
    def _prepare_web_index_content(self, context: WebIndexContext) -> str:
        ...
        Args:
            context (WebIndexContext): Context object with all necessary data.

        Returns:
            str: The complete Markdown content for the index file.
        """
        ...
        content_parts.append("\n".join(chapter_links) if chapter_links else "No chapters generated for this summary.")
        return "\n".join(content_parts)
    ...
    def _prepare_standard_web_chapters_data(
        ...
        Args:
            ...
            chapter_order (WebChapterOrderList): Ordered list of concept indices.
            chapters_content (WebChapterContentList): List of generated Markdown for each chapter.

        Returns:
            AllWebChaptersDataList: A list of dictionaries, each representing a standard chapter.
        """
            ...
            std_chapters.append(
                {
                    ...
                    "filename": "",
                    "num": -1,  # num and filename filled later
                }
            )
    ...
    def _prepare_special_web_chapter_data(
        ...
        Args:
            content_key (str): Key in shared_context holding the chapter's Markdown content.
            title (str): The title of this special chapter.
            chapter_type (WebChapterTypeLiteral): The type of the special chapter.
            shared_context (SLSharedContext): The shared context dictionary.

        Returns:
            Optional[WebChapterFileData]: A dictionary with chapter data if content is found, else None.
        """
        ...
        self._log_info("'%s' web chapter data prepared (len: %d).", chapter_type.capitalize(), len(content_val_str))
        return {
            "filename": "",
    ...
    def _assemble_all_web_chapters(
        ...
        Args:
            shared_context (SLSharedContext): The main shared context.
            data (SharedDataForWebCombine): Data retrieved specifically for this node.

        Returns:
            AllWebChaptersDataList: A list of all chapter data, sorted and with filenames.
        """
        all_chapters: AllWebChaptersDataList = []
        cfg: ConfigDictTyped = cast(ConfigDictTyped, data.get("config", {}))
        out_cfg: OutputConfigDictTyped = cast(OutputConfigDictTyped, cfg.get("output", {}))
        ...
        if bool(out_cfg.get("include_source_index", False)):  # Reusing flag
        ...
        if bool(out_cfg.get("include_project_review", False)):  # Reusing flag
        ...
        for chapter_info in all_chapters:
            name_raw = str(chapter_info.get("name", f"web-chapter-{chapter_info.get('num')}"))
            ...
            base_name = (
                ...
                else (sanitize_filename(name_raw) or f"chapter-{chapter_info.get('num')}")
            )
    ...
    def _retrieve_shared_data_for_web_combine(self, shared_context: SLSharedContext) -> SharedDataForWebCombine:
        ...
        Returns:
            SharedDataForWebCombine: A dictionary of required data.
    ...
    def _initialize_output_context(
        ...
        Args:
            shared_context (SLSharedContext): The main shared context.
            retrieved_data (SharedDataForWebCombine): Data specific to this node.

        Returns:
            tuple[WebFooterInfo, Path]: Footer information and the resolved output path.
        """
        ...
        if output_path_str_from_context and isinstance(output_path_str_from_context, str):
            ...
            web_opts_val_prep: Any = (
                config_val_prep.get("web_crawler_options", {}) if isinstance(config_val_prep, dict) else {}
            )
            web_opts_prep: dict[str, Any] = web_opts_val_prep if isinstance(web_opts_val_prep, dict) else {}
            default_subdir = str(web_opts_prep.get("default_output_subdir_name", "crawled_web_content"))
    ...
    def _write_all_files(
        ...
        Args:
            output_path (Path): The base output directory for this summary.
            index_content (str): The Markdown content for the index file.
            all_chapters (AllWebChaptersDataList): List of all chapter data.
            footer_str (str): The footer string to append if not present.
        ...
        if WEB_FOOTER_SEPARATOR not in final_index_content:  # Append footer if not already there
        ...
        for ch_info in all_chapters:
            ...
            content = str(ch_info.get("content", ""))  # Should already have footer from _add_footers
            if fname and content:
                try:
                    ...
                    all_chapter_writes_ok = False  # Mark but continue
            else:
                self._log_warning("Skipping chapter write due to missing filename or content: %s", ch_info.get("name"))
    ...
    def pre_execution(self, shared_context: SLSharedContext) -> CombineWebSummaryPreparedInputs:
        ...
        Returns:
            CombineWebSummaryPreparedInputs: True if successful, False otherwise.
        """
        self._log_info(">>> CombineWebSummary.pre_execution: Assembling and writing web summary files. <<<")
        output_path_for_summary: Optional[Path] = None  # Initialize for finally block
        try:
            ...
            if not all_chapters:
                self._log_warning("No web chapters assembled. Output will be minimal (index only).")
            ...
            self._add_navigation_links(all_chapters, WEB_INDEX_FILENAME)  # Modifies all_chapters in-place
            self._add_footers(all_chapters, footer_str)  # Modifies all_chapters in-place

            write_success = self._write_all_files(output_path_for_summary, index_md_content, all_chapters, footer_str)
            shared_context["final_output_dir"] = str(output_path_for_summary) if write_success else None
            return write_success
        except (ValueError, LlmApiError, OSError) as e_crit:  # LlmApiError might come from dependencies if misused
            self._log_error("CombineWebSummary pre_execution critical error: %s", e_crit, exc_info=True)
            if output_path_for_summary:  # Log where it attempted to write
                self._log_warning(
                    "Attempted to write to output directory: %s. Check logs.", output_path_for_summary.resolve()
                )
            return False  # Indicate failure
        finally:
            self._log_info(">>> CombineWebSummary.pre_execution: Exit. <<<")
    ...
    def execution(self, prepared_inputs: CombineWebSummaryPreparedInputs) -> CombineWebSummaryExecutionResult:
        ...
        Args:
            prepared_inputs (CombineWebSummaryPreparedInputs): Result from pre_execution.

        Returns:
            CombineWebSummaryExecutionResult: Always None.
        """
    ...
    def post_execution(
        ...
        Args:
            shared_context (SLSharedContext): The shared context dictionary.
            prepared_inputs (CombineWebSummaryPreparedInputs): Result from pre_execution.
            execution_outputs (CombineWebSummaryExecutionResult): Result from execution (None).
        """
        ...
        if prepared_inputs and final_dir_val and Path(final_dir_str).exists():
            ...
            status_msg = "completed (output dir may not be confirmed or no content written)"
        else:
        ...
        log_msg_l2 = f"Final output dir set in shared_context: {final_dir_str}"
        self._log_info(log_msg_l1 + log_msg_l2)


# End of src/sourcelens/nodes/web/n08_combine_web_summary.py
