...
from sourcelens.prompts.web.chapter_prompts import WebChapterPrompts  # Updated import path
...
WebContentConceptItem: TypeAlias = dict[str, Any]  # From n21
WebContentConceptsList: TypeAlias = list[WebContentConceptItem]  # From n21
WebContentRelationshipsDict: TypeAlias = dict[str, Any]  # From n22
...
class OrderWebChapters(BaseNode[OrderWebChaptersPreparedInputs, OrderWebChaptersExecutionResult]):
    ...
    This node uses identified web concepts and their analyzed relationships
    to suggest a logical sequence for chapters.
    """
    ...
    def _parse_single_index_entry(self, entry: RawLLMIndexEntry, position: int) -> int:
        ...
        Args:
            entry (RawLLMIndexEntry): The raw entry from the YAML list.
            position (int): The zero-based position of the entry in the list, for error reporting.

        Returns:
            int: The parsed integer index.
    ...
    def _parse_and_validate_web_order(self, ordered_indices_raw: list[Any], num_concepts: int) -> WebChapterOrderList:
        ...
        Args:
            ordered_indices_raw (list[Any]): The raw list parsed from YAML by `validate_yaml_list`.
            num_concepts (int): The expected number of concepts/chapters.

        Returns:
            WebChapterOrderList: A validated list of integer indices for chapter order.

        Raises:
            ValidationFailure: If validation checks fail (e.g., wrong count, duplicates, out of range).
        """
        if not isinstance(ordered_indices_raw, list):  # Should be caught by validate_yaml_list
    ...
    def _build_web_chapters_context(
        ...
        Args:
            ...
            relationships (WebContentRelationshipsDict): Dictionary of web relationship details.
    ...
    def pre_execution(self, shared_context: SLSharedContext) -> OrderWebChaptersPreparedInputs:
        ...
        Returns:
            OrderWebChaptersPreparedInputs: A dictionary for the `execution` method.
        """
        ...
        prepared_data: OrderWebChaptersPreparedInputs = {
            "skip": not text_concepts,
            "reason": "No web concepts provided for chapter ordering." if not text_concepts else "",
        ...
        if not text_concepts:
            self._log_warning("No web concepts found. Chapter order cannot be determined by LLM.")
            return prepared_data

        concepts_listing_str, rels_summary_str = self._build_web_chapters_context(text_concepts, text_relationships)
    ...
    def execution(self, prepared_inputs: OrderWebChaptersPreparedInputs) -> OrderWebChaptersExecutionResult:
        ...
        Args:
            prepared_inputs (OrderWebChaptersPreparedInputs): From `pre_execution`.

        Returns:
            OrderWebChaptersExecutionResult: A list of ordered concept indices.
        ...
        prompt = WebChapterPrompts.format_order_web_chapters_prompt(
            ...
            relationships_summary=prepared_inputs["relationships_summary"],
            num_concepts=num_concepts,
        ...
        try:
            # Schema for a list of items, where each item can be int, string, or number
    ...
    def post_execution(
        ...
        Args:
            ...
            prepared_inputs (OrderWebChaptersPreparedInputs): Result from `pre_execution`.
            execution_outputs (OrderWebChaptersExecutionResult): List of ordered concept indices.
        """
        del prepared_inputs
        shared_context["text_chapter_order"] = execution_outputs  # New key
        self._log_info("Stored web chapter order in shared_context['text_chapter_order']: %s", execution_outputs)


# End of src/sourcelens/nodes/web/n04_order_web_chapters.py
