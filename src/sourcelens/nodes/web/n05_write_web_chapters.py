...
from sourcelens.prompts.web.chapter_prompts import WebChapterPrompts, WriteWebChapterContext
...
MAX_SNIPPET_LEN_FOR_CHAPTER_PROMPT: Final[int] = 1000  # Local constant for this node
EXPECTED_FILE_DATA_TUPLE_LENGTH_WEB: Final[int] = 2  # Constant for tuple length
...
class WriteWebChapters(BaseBatchNode[WriteWebChapterPreparedItem, SingleWebChapterExecutionResult]):
    ...
    def _get_relevant_snippets_for_concept(
        self, concept_item: WebContentConceptItem, files_data: FileDataListInternal
    ) -> str:
        """Retrieve and concatenate content snippets for a given concept.
        ...
        """
        source_doc_paths_raw: Any = concept_item.get("source_documents", [])
        source_doc_paths: list[str] = (
            [str(p) for p in source_doc_paths_raw if isinstance(p, str)]
            if isinstance(source_doc_paths_raw, list)
            else []
        )

        if not source_doc_paths:
            return "No specific source documents were linked to this concept during identification."

        content_by_path: dict[str, str] = {path: content for path, content in files_data}
        relevant_snippets: list[str] = []

        for doc_path_key in source_doc_paths:
            if doc_path_key in content_by_path:
                content = content_by_path[doc_path_key]
                snippet = content[:MAX_SNIPPET_LEN_FOR_CHAPTER_PROMPT]
                if len(content) > MAX_SNIPPET_LEN_FOR_CHAPTER_PROMPT:
                    snippet += "..."
                relevant_snippets.append(f"--- Snippet from: {doc_path_key} ---\n{snippet}\n\n")
            else:
                self._log_warning(
                    "Document '%s' listed in concept '%s' not found in crawled files_data. Snippet skipped.",
                    doc_path_key,
                    concept_item.get("name", "Unknown Concept"),
                )

        if not relevant_snippets:
            return "Could not retrieve content for any of the source documents linked to this concept."
    ...
    def _get_context_from_shared(self, shared_context: SLSharedContext) -> dict[str, Any]:
        ...
        if isinstance(files_data_any, list):
            for item in files_data_any:
                if (
                    isinstance(item, tuple)
                    and len(item) == EXPECTED_FILE_DATA_TUPLE_LENGTH_WEB
                    and isinstance(item[0], str)
                    and isinstance(item[1], str)
                ):
                    files_data.append(cast(FileDataInternal, item))
        if len(files_data_any or []) != len(files_data):
            self._log_warning("Some items in shared_context['files'] were invalid for web chapter prep.")
    ...
    def pre_execution(self, shared_context: SLSharedContext) -> Iterable[WriteWebChapterPreparedItem]:  # type: ignore[override]
        ...
        Yields:
            WriteWebChapterPreparedItem: Dictionaries with context for single chapter generation.
        """
        ...
        files_data: FileDataListInternal = context_data["files_data"]
        ...
        if not files_data:
            self._log_warning(
                "No crawled file content available in shared_context['files']. "
                "Web chapters might lack relevant snippets."
            )
        ...
        for i, concept_idx_for_chapter in enumerate(chapter_order):
            ...
            relevant_snippets_str = self._get_relevant_snippets_for_concept(current_concept_item, files_data)
    ...
    def _call_llm_for_web_chapter_with_retry(
        ...
        Args:
            ...
            llm_config (LlmConfigDictTyped): Configuration for the LLM API.
        ...
        Returns:
            str: Raw string content from LLM or a formatted error message.
        """
    ...
    def _process_single_web_chapter_item(self, item: WriteWebChapterPreparedItem) -> SingleWebChapterExecutionResult:
        ...
        Args:
            item (WriteWebChapterPreparedItem): Dictionary with context for one chapter.

        Returns:
            SingleWebChapterExecutionResult: Markdown content or error message string.
        """
        try:
    ...
    def execution(self, items_iterable: Iterable[WriteWebChapterPreparedItem]) -> WriteWebChaptersExecutionResultList:
        ...
        Args:
            items_iterable (Iterable[WriteWebChapterPreparedItem]): Iterable of prepared chapter items.

        Returns:
            WriteWebChaptersExecutionResultList: List of Markdown strings (chapter content or errors).
        """
    ...
    def post_execution(
        ...
        Args:
            shared_context (SLSharedContext): The shared context dictionary.
            prepared_items_iterable (Iterable[WriteWebChapterPreparedItem]): Prepared items (unused).
            execution_results_list (WriteWebChaptersExecutionResultList): Generated chapter contents.
        """
...
# End of src/sourcelens/nodes/web/n05_write_web_chapters.py
