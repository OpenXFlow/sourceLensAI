...
# Import guidelines for inline diagrams (assuming it's generally applicable)
# Adjust path if a web-specific guideline is created later.
from sourcelens.prompts.code.diagrams import INLINE_MERMAID_DIAGRAM_GUIDELINES_TEXT

# Type Aliases
WebChapterMetadata: TypeAlias = dict[str, Any]  # keys: num, name, filename, concept_index
...
class WriteWebChapterContext:
    ...
    Attributes:
        document_collection_name (str): Name of the website/document collection.
        chapter_num (int): The sequential number of this chapter.
        concept_name (str): The core concept/topic this chapter covers.
        concept_summary (str): A summary of the concept.
        full_chapter_structure_md (str): Markdown formatted list of all planned chapters.
        relevant_document_snippets (str): String containing relevant text snippets
                                          from original web documents for this concept.
        target_language (str): Target language for the chapter content.
        prev_chapter_meta (Optional[WebChapterMetadata]): Metadata of the preceding chapter.
        next_chapter_meta (Optional[WebChapterMetadata]): Metadata of the succeeding chapter.
    """
...
class WebChapterPrompts:
    ...
    def format_order_web_chapters_prompt(
        ...
        ordering_criteria_intro = (
            "Based on the identified concepts and their relationships, determine a logical and "
            "pedagogical order for chapters that would summarize this web content effectively."
        )
        ...
        prompt_lines: list[str] = [
            ...
            "The following core concepts/topics have been identified from the content:",
    ...
    def _prepare_web_chapter_transitions(context: WriteWebChapterContext) -> tuple[str, str]:
        ...
        Args:
            context (WriteWebChapterContext): Context for the current web chapter.
    ...
    def format_write_web_chapter_prompt(context: WriteWebChapterContext) -> str:
        """Format prompt for LLM to write a single chapter based on web content.

        Args:
            context (WriteWebChapterContext): Dataclass containing all necessary
                                              information for generating the chapter.
        ...
        instr_elaboration = (
            ...
            "Use the 'Relevant Document Snippets' to extract key information, facts, "
            "and explanations. Synthesize and rephrase; do NOT just copy. "
            "Aim for 3-5 well-structured paragraphs."
        )
        instr_examples = (
            f"4.  **Key Points/Examples (if applicable){hints['instr_note']}:** "
            "If the snippets contain clear examples or bullet points illustrating the concept, "
            "include 1-2 concise examples or a short bulleted list. Keep it brief."
        )
        # Using the imported INLINE_MERMAID_DIAGRAM_GUIDELINES_TEXT
        instr_diagrams = (
        ...
        instr_tone = (
            ...
            "for someone learning about this content. Avoid jargon where possible or explain it."
        )
        instr_conclusion_l1 = (
        ...
        prompt_lines: list[str] = [
            ...
            f"\n**Relevant Document Snippets for '{context.concept_name}' "
            "(use these to elaborate on the concept):\n**"
...
# End of src/sourcelens/prompts/web/chapter_prompts.py
