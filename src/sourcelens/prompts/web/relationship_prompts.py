...
# Type Aliases to match node data structures if they were more specific
...
class WebRelationshipPrompts:
    ...
    def format_analyze_web_relationships_prompt(
        ...
        Args:
            ...
            concepts_listing_with_summaries (str): A string listing the identified web concepts,
                                                   formatted typically as "Index. Name - Summary".
                                                   This provides context for relationship analysis.
            num_concepts (int): The total number of concepts identified, used for validating
                                the indices in the LLM's response.
            target_language (str): The target language for the `overall_summary` and
                                   relationship `label` fields in the output.
            max_relationships (int): A suggestion to the LLM for the maximum number of
                                     key relationships to detail. Defaults to 10.

        Returns:
            str: A formatted multi-line string constituting the complete prompt for the LLM.
        """
        lang_instr_suffix = ""
        ...
        # Assemble the YAML example structure ensuring no line is too long
        yaml_parts = [
            "```yaml",
            yaml_ex_summary_l1,
            yaml_ex_rels_header,
...
# End of src/sourcelens/prompts/web/relationship_prompts.py
